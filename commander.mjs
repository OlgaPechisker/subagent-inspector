    ...
    return {
        ...
        handler: async (context) => {
            ...
            if (prefix) {
                ...
                // Post in ≤800-char chunks to stay within session.log limits
                const lines = content.split('\n');
                let chunk = '';
                for (const line of lines) {
                    if ((chunk + line + '\n').length > 800) {
                        await session.log(chunk.trimEnd());
                        chunk = '';
                    }
                    chunk += line + '\n';
                }
                ...
                const list = logs.map((l, i) => `${i + 1}. ${l.name}`).join('\n');
                await session.log(`Recent subagent logs (${logDir}):\n${list}`);
            }
        },
    };
}
